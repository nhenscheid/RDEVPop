...
subplot(2,2,3); imagesc(gbar);set(gca,'YDir','normal'); axis image; 
subplot(2,2,4); imagesc(g);set(gca,'YDir','normal'); axis image; drawnow; 

objfun = @(X) poiss_obj_fun_cov(g,X,L,blur_kernel);
maxiter = 50000; 

options = optimoptions(@fmincon,'Display','iter-detailed','MaxFunctionEvaluations',maxiter,'Algorithm','interior-point','OptimalityTolerance',1e-8,'OutputFcn',@(x,optimValues,state)opt_plot_fun(x,optimValues,state,optplotfig,L0));
LB = [0*ones(2*K_recon,1);0;0];
UB = [1*ones(2*K_recon,1);1;.01]; 
optTime = tic;
