...
%%  Not randomizing anything, rho drawn from posterior
Nsamp = 512; 
RMCMC = RDE; 
RMCMC.kappa = L_kappa; 
RMCMC.D = L_D; 
...
for i=1:Nsamp
    fprintf('%i/%i\n',i,Nsamp); 
    RMCMC.rho = L_array{i}; 
    nMCMC{i} = RMCMC.Solve(t);
end 
    
%%  Randomizing everything, rho drawn from posterior
Nsamp = 512; 
RMCMC = RDE; 
RMCMC.kappa = L_kappa; 
RMCMC.D = L_D; 
RMCMC.u0 = u0; 
RMCMC.grid = {xx,xx}; 
nMCMC_all = cell(Nsamp,1); 
for i=1:Nsamp
    fprintf('%i/%i\n',i,Nsamp); 
    RMCMC.rho = L_array{i}; 
    RMCMC.kappa.Randomize; 
    RMCMC.D.Randomize; 
    nMCMC_all{i} = RMCMC.Solve(t);
end 
